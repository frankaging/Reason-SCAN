...
from GCN_seq2seq.cnn_model import ConvolutionalNet
from GCN_seq2seq.cnn_model import DownSamplingConvolutionalNet
from GCN_seq2seq.seq2seq_model import EncoderRNN
from GCN_seq2seq.seq2seq_model import Attention
from GCN_seq2seq.seq2seq_model import LuongAttentionDecoderRNN
from GCN_seq2seq.seq2seq_model import BahdanauAttentionDecoderRNN
from GCN_seq2seq.gnn import LGCNLayer
...
class Model(nn.Module):
    ...
    def __init__(self, input_vocabulary_size: int, embedding_dimension: int, encoder_hidden_size: int,
        ...
        # some layers to transform situations into GCN embeddings
        self.gcn_upscale_embedding = nn.Linear(17, 16*4, bias=False)  # 64
        self.gcn_downscale_embedding = nn.Linear(16*4, 17, bias=False)  # 64
        self.lgcn = LGCNLayer()
    ...
    def nonzero_insertor(self, node_out, situation_batch):
        ...
        situation_paddng = torch.zeros(B, H, W, padding_len).to(situation_batch.device)
        situation_batch = torch.cat((situation_batch, situation_paddng), dim=-1)
        for i in range(situation_batch.size(0)):
            sum_x = torch.sum(situation_batch[i], dim=-1)
    ...
    def encode_input(self, commands_input: torch.LongTensor, commands_lengths: List[int],
        ...
        gcn_situations_input = self.gcn_upscale_embedding(situations_input)
        gcn_situations_node_embeddings = self.nonzero_extractor(situations_input, gcn_situations_input)
        
        batchSize = gcn_situations_input.size(0)
        gs = []
        graph_membership = []
        dgl_gs = [dgl.DGLGraph() for _ in range(batchSize)]
        for i, x in enumerate(gcn_situations_node_embeddings):
            dgl_gs[i] = dgl_gs[i].to(device)
        ...
        gcn_situations_node_embeddings = torch.cat(gcn_situations_node_embeddings, dim=0)
        graph_membership = torch.tensor(graph_membership, dtype=torch.long, device=device)

        # LGCN
        commands_lengths_t = torch.tensor(commands_lengths)
        commands_lengths_t = commands_lengths_t.to(device)
        situation_out_node = self.lgcn(
            gcn_situations_node_embeddings, 
            batch_g, hidden, encoder_outputs["encoder_outputs"].transpose(0,1), 
            commands_lengths_t, batchSize,
            graph_membership
        )
        situation_batch = self.nonzero_insertor(situation_out_node, situations_input)
        situation_batch = self.gcn_downscale_embedding(situation_batch)
        
        encoded_image = self.situation_encoder(situation_batch)
